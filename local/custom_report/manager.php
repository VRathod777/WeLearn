...
JOIN {user_info_field} f1 ON d1.fieldid = f1.id  where shortname = 'Entity' AND data LIKE :entity)";
    $params['entity'] = '%' . $entity . '%';
}
if (!empty($department)) {
    $where .= " AND u.id IN(SELECT d1.userid from {user_info_data} d1 
JOIN {user_info_field} f1 ON d1.fieldid = f1.id  where shortname = 'Department' AND data LIKE :department)";
...
// just manager condition...
if($USER->email){
    $where .= " AND uid.userid in(
    SELECT d.userid FROM {user_info_data} d WHERE d.fieldid = 6 
    AND d.data = '$USER->email')";
}
...
// Page setup
$PAGE->set_url('/local/custom_report/manager.php', [
    'entity' => $entity,
...
// Navigation
$PAGE->navbar->add(get_string('report_title', 'local_custom_report'), new moodle_url('/local/custom_report/manager.php'));
...
echo '<a href="' . $CFG->wwwroot . '/local/custom_report/manager.php" class="btn btn-secondary">' . get_string('reset') . '</a>';
echo '</div>';
...
echo '<br>';
// Add export button
$exporturl = new moodle_url('/local/custom_report/manager.php', array(
    'download' => 'csv',
    'entity' => $entity,
    'department' => $department,
    // 'sbu' => $sbu,
...
$table->no_sorting('Entity');
$table->no_sorting('Department');
// $table->no_sorting('SBU');

$baseurl = new moodle_url('/local/custom_report/manager.php', array(
    'entity' => $entity,
    'department' => $department,
    // 'sbu' => $sbu,
    'trainingname' => $trainingname,
...
GROUP BY u.id, c.id
ORDER BY u.username";
$records6 = $DB->get_records_sql($sql65, $params);
