...
/**
 * Page to revoke and disable an message code.
 *
...
$url = new moodle_url('/admin/tool/mfa/factor/message/message.php',
    ['instance' => $instanceid, 'pass' => $pass, 'secret' => $secret]);
$PAGE->set_url($url);
$PAGE->set_pagelayout('secure');
$PAGE->set_title(get_string('unauthmessage', 'factor_message'));
$PAGE->set_cacheable(false);
$instance = $DB->get_record('tool_mfa', ['id' => $instanceid]);
$factor = \tool_mfa\plugininfo\factor::get_factor('message');
...
if (!empty($instance) && $pass != 0 && $secret != 0) {
    ...
    if ($instance->secret != $secret) {
        ...
        throw new moodle_exception('error:parameters', 'factor_message');
    }
    $factor = \tool_mfa\plugininfo\factor::get_factor('message');
...
$form = new \factor_message\form\message($url);

if ($form->is_cancelled()) {
    redirect(new moodle_url('/'));
} else if ($fromform = $form->get_data()) {
    if (empty($instance)) {
        $message = get_string('error:badcode', 'factor_message');
    } else {
        $user = $DB->get_record('user', ['id' => $instance->userid]);

        // Stop attacker from using message factor at all, by revoking all message until admin fixes.
        $DB->set_field('tool_mfa', 'revoked', 1, ['userid' => $user->id, 'factor' => 'message']);
        ...
        $event = \factor_message\event\unauth_message::unauth_message_event($user, $ip, $useragent);
        $event->trigger();

        // Suspend user account.
        if (get_config('factor_message', 'suspend')) {
            $DB->set_field('user', 'suspended', 1, ['id' => $user->id]);
        }

        $message = get_string('message:revokesuccess', 'factor_message', fullname($user));
    }
}

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('unauthmessage', 'factor_message'));
if (!empty($message)) {
    echo $message;
} else {
    $form->display();
